(function () {
  ...
  var GREET = [
    ...
    [
      S(7),
      L("nar", "День 6. День рождения Сонечки!"),
      W(800),
      L("nar", "Сегодня праздник — и самое важное приключение только начинается."),
      W(600),
      L("gru", "Я есть Грутик!", "Сегодня твой день!"),
      L("venom", "А мы — Веном. С днём рождения."),
      L("nar", "Они стоят плечом к плечу. Точнее, веточкой к щупальцу. Сколько всего вы прошли вместе!"),
      L("venom", "Скоро откроется сегодняшний секрет."),
      L("gru", "Я есть Грутик!", "Я так ждал этого дня!"),
      W(500),
      L("nar", "Грутик осторожно касается колеса. Оно тихонько гудит, но не вращается."),
      F("wheelBroken"),
      L("nar", "Колесо заблокировано! Вчера питание восстановили, но для праздничного запуска обычной мощности не хватает."),
      L("venom", "Сработала система защиты. Запуск по особому протоколу — «Протокол SONECHKA»."),
      L("gru", "Я есть Грутик!", "Мы снимем защиту вместе!"),
      L("venom", "Система откроется только той, кто прошла все дни и испытания."),
      C("day6_ready", "Готова к праздничному запуску?", [
        { text: "🚀 Готова! Запускаем!", steps: [
          L("gru", "Я ЕСТЬ ГРУТИК!", "НА ПОЛНУЮ МОЩНОСТЬ!"),
          L("venom", "Мы готовы. Поехали!"),
          L("nar", "Энергия пульсирует, все огоньки вспыхивают ярким светом!")
        ]},
        { text: "😬 Немного волнуюсь...", steps: [
          L("gru", "Я есть Грутик.", "Я с тобой. Не бойся!"),
          L("nar", "Грутик протягивает маленькую веточку и нежно кладёт её тебе на ладонь."),
          L("venom", "Не волнуйся. Мы рядом."),
          L("nar", "В такой компании бояться точно нечего.")
        ]}
      ]),
      L("nar", "Пройди синхронизацию всех систем, чтобы снять защиту, раскрутить колесо и открыть сегодняшний секрет!"),
