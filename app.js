(function () {
  ...
  var dayGames = [
    ...
    { type: "finale", mark: "06", icon: "🎂", badge: "Праздник", desc: "3 праздничных этапа с фото", kicker: "ДЕНЬ 06 · ПРАЗДНИК", title: "Праздничный механизм", instruction: "Грутик и Веном спрятали фотографии. Распакуй их!" }
  ];
  ...
  function dayToStage(d) {
    if (window.__previewDay != null) {
      ...
      if (d === 1) return 2;
      if (d === 2) return 3;
      if (d === 3) return state.venomControlled ? 6 : 5;
      if (d === 4) return 7;
  ...
  var GREET = [
    ...
    [
      S(4.5),
      L("sonechka", "Грутик?.. Почему ты на потолке?"),
      W(500),
      L("nar", "Над горшком шевелится тёмный кокон. Внутри, обмотанный щупальцами, висит Грутик."),
      L("gru", "Я ЕСТЬ ГРУТИК!!!", "Потому что эта чёрная штука меня схватила! Сними меня!"),
      L("venom", "Мы нашли дом."),
      L("gru", "Я есть Грутик!", "Вообще-то в гости сначала стучатся!"),
      L("sonechka", "Для начала поставь его на землю."),
      L("nar", "Щупальце разжимается. Грутик шлёпается в горшок, но чёрная живая материя уже растекается по его коре."),
      S(4),
      F("blobIn", 300),
      FN(infect),
      F("blackout", 900),
      S(5),
      L("nar", "Белые глаза вспыхивают над знакомым личиком. Веточки и чёрные нити забавно дёргаются в разные стороны."),
      L("venom", "Вы вчера сказали: «мы». Мы пришли."),
      L("gru", "Я есть Грутик.", "Но это не значит, что можно мной командовать!"),
      C("day4_venom", "Что ты сделаешь?", [
        { text: "🌿 Грутик, смотри на меня. Ты здесь", steps: [
          L("nar", "Грутик цепляется за твой взгляд. Дрожь в веточках сразу утихает."),
          L("gru", "Я есть Грутик.", "Я здесь. Всё хорошо!"),
          L("venom", "Мы слышим.")
        ]},
        { text: "🖤 Веном, разожми щупальце. Сейчас", steps: [
          L("nar", "Чёрная нить слегка натягивается, но всё-таки послушно отпускает веточку."),
          L("venom", "Мы... отпустили."),
          L("gru", "Я есть Грутик.", "Фух, так намного лучше!")
        ]}
      ]),
      L("sonechka", "Так, хватит ссориться! Вы оба в одном горшке, учитесь дружить."),
      L("nar", "Грутик и Веном начинают препираться и случайно задевают вашу общую фотографию..."),
      F("flash", 250),
      L("nar", "Она падает и разлетается на мелкие кусочки пазла!"),
      L("gru", "Я есть Грутик...", "Ой-ой... Это же любимое фото Вадима и Сонечки..."),
      L("venom", "Мы не хотели сломать. Мы просто держали."),
      L("gru", "Я есть Грутик!", "Слишком сильно держали!"),
      L("sonechka", "Так, спокойно. Сначала распутаем щупальца, а потом вместе соберём фотографию."),
      B("Собрать нашу фотографию", markSeen(3))
    ],
    [
      S(7),
      L("gru", "Я есть Грутик!", "Мы научились ладить!"),
      L("venom", "Мы есть Грутик."),
      L("nar", "Они стоят рядышком — веточки и щупальца аккуратно переплелись. Кажется, эти двое наконец подружились."),
      W(600),
      L("nar", "Грутик указывает на колесо. Оно погасло — энергия не поступает."),
      L("venom", "Питание отключилось. Контур разомкнут."),
      L("nar", "Ветви и чёрные щупальца тянутся к проводам, но те искрят — цепь повреждена."),
      L("gru", "Я есть Грутик...", "Колесо сломалось, подарки не крутятся..."),
      C("day5_repair", "Как будем чинить?", [
        { text: "🔧 Покажите мне схему", steps: [
          L("venom", "Разумный подход. Вот схема, смотри."),
          L("nar", "На экране загорается электрическая схема. Узлы светятся, но некоторые развёрнуты не туда.")
        ]},
        { text: "⚡ Просто подайте питание!", steps: [
          L("nar", "Грутик пускает импульс по ветке. Яркая искра! Щелчок!"),
          L("venom", "Мы же говорили — замкнёт! Нужна схема."),
          L("gru", "Я есть Грутик...", "Ой-ой... кажется, пробки выбило..."),
          L("nar", "Ладно, сначала повернём узлы — потом включим.")
        ]}
      ]),
      L("nar", "Сонечка, помоги им повернуть узлы схемы и вернуть колесу питание!"),
      B("Починить схему", markSeen(4))
    ],
    [
      S(7),
      L("gru", "Я ЕСТЬ ГРУТИК!", "С днём рождения, Сонечка!"),
      F("heartsIn", 400),
      W(600),
      L("venom", "С днём рождения."),
      L("nar", "Грутик подпрыгивает в горшке так, что щупальца Венома еле удерживают его на месте."),
      L("gru", "Я есть Грутик!", "Мы готовили это всю ночь! Ну, почти всю..."),
      L("venom", "Он не спал. Я тоже. Мы не спали."),
      L("gru", "Я есть Грутик...", "Ну ладно, я немножко дремал."),
      W(500),
      L("nar", "Грутик тянется к колесу. Оно тихонько гудит, но обод не двигается."),
      F("wheelBroken"),
      L("gru", "Я есть Грутик?", "Э-э... почему оно не крутится?"),
      L("venom", "Защита. Вчера мы починили питание, но колесо заблокировано."),
      L("nar", "На ободе мерцает надпись: «Протокол SONECHKA»."),
      L("gru", "Я есть Грутик!", "О! Это же наш праздничный механизм!"),
      L("venom", "Мы его собрали вместе. Четыре замка — ростки, тёплая тень, общий ритм и колесо запуска."),
      L("gru", "Я есть Грутик.", "Откроешь все четыре — и сегодняшний секрет твой."),
      C("day6_ready", "Ну что, попробуем?", [
        { text: "🚀 Конечно! Поехали!", steps: [
          L("gru", "Я ЕСТЬ ГРУТИК!", "Вот это настрой!"),
          L("venom", "Поехали."),
          L("nar", "Грутик от радости роняет горшок. Веном ловит его щупальцем на лету.")
        ]},
        { text: "😬 А вдруг я не справлюсь?", steps: [
          L("gru", "Я есть Грутик.", "Ты справишься. Ты же Сонечка."),
          L("nar", "Он протягивает маленькую веточку и кладёт её тебе на ладонь."),
          L("venom", "Мы рядом. Ничего сложного."),
          L("nar", "Ладно. С такой поддержкой точно получится.")
        ]}
      ]),
      L("nar", "Открой четыре замка, чтобы раскрутить колесо и отпраздновать день рождения!"),
      B("Запустить механизм", markSeen(5))
    ]
  ];
  ...
  var POSTWIN = [
    ...
    [
      ...
      L("gru", "Я есть Грутик.", "Ладно. Только больше не хулигань и не хватай без спроса."),
      L("venom", "Договорились. Можно помочь с колесом?"),
      L("gru", "Я есть Грутик!", "Теперь можно!"),
      FN(controlVenom),
      S(6),
      F("wheelIn"),
      L("nar", "Веточка толкает обод, а щупальце ловко подкручивает колесо. Оно вращается легко и плавно!"),
      L("venom", "Вместе."),
      B("Крутить колесо")
    ],
    [
    ...
    [
      FN(unlockFinal),
      F("glow", 400),
      L("nar", "Последний замок открывается. Тёплый свет заливает механизм и проходит сквозь Грутика."),
      W(900),
      L("nar", "Что-то меняется. Чёрная масса на секунду отделяется — и рядом с ней стоит обычный маленький Грутик."),
      L("venom", "Мы можем уйти. Если хочешь."),
      L("gru", "Я есть Грутик.", "Подожди."),
      W(400),
      L("gru", "Я есть Грутик.", "Ты ведь тоже привык."),
      L("venom", "...Привык?"),
      L("gru", "Я есть Грутик.", "Ну да. К нам. К горшку. К Сонечке."),
      C("venom_stay", "Что скажешь Веному?", [
        { text: "🤝 Оставайся. Вы — команда.", steps: [
          L("venom", "...Ладно."),
          L("nar", "Щупальца осторожно обнимают ветки. Как будто боятся сломать."),
          L("gru", "Я есть Грутик!", "Вот и договорились.")
        ]},
        { text: "💚 Грутик сам решит", steps: [
          L("gru", "Я есть Грутик.", "Оставайся. Вместе веселее."),
          L("venom", "Он сказал — остаёмся."),
          L("nar", "Грутик кивает. Веном кивает. Все кивают.")
        ]}
      ]),
      L("nar", "Симбиот возвращается. Тихо. Аккуратно. Форма теперь другая — мягче, спокойнее."),
      F("merge", 300),
      F("wheelIn"),
      L("gru", "Я есть Грутик!", "А теперь — подарок!"),
      B("Крутить вместе")
    ]
  ];
  ...
  var AFTERGIFT = [
    ...
    [
      S(6),
      L("nar", "Секретная коробочка открыта. Рядом в красивой рамке стоит ваша собранная фотография."),
      L("venom", "Почему у Вадима и Сонечки на головах боевые конусы?"),
      L("gru", "Я есть Грутик.", "Это праздничные колпачки! На праздниках их надевают для веселья."),
      L("venom", "Мы тоже хотим конус."),
      L("sonechka", "А волшебное слово?"),
      W(500),
      L("venom", "...Пожалуйста."),
      L("nar", "Тонкое щупальце сворачивается треугольником на макушке. Грутик примеряет импровизированный колпачок, а Веном довольно щурится."),
      C("day4_after", "Что скажешь этим двоим?", [
        { text: "🎉 Вам идёт. Обоим!", steps: [
          L("gru", "Я есть Грутик!", "Ура! Мы готовы к празднику!"),
          L("venom", "Боевой конус одобрен.")
        ]},
        { text: "🌙 Только спать будете не на потолке", steps: [
          L("venom", "Договорились. На потолке неудобно."),
          L("gru", "Я есть Грутик.", "Сегодня спим мирно в горшке.")
        ]}
      ]),
      L("nar", "У основания колеса фортуны пробегает маленькая искорка. Похоже, где-то отошёл контакт."),
      L("venom", "Схема барахлит. Завтра починим. Вместе."),
      B("До завтра!")
    ],
    [
      S(7),
      ...
      L("gru", "Я есть Грутик!", "Завтра твой день рождения!"),
      L("venom", "И у нас есть ещё один секрет."),
      L("nar", "Грутик сияет от нетерпения и едва усидит в горшке."),
      B("До завтра!")
    ]
  ];
