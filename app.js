(function () {
  ...
  var dayGames = [
    ...
    { type: "photoPuzzle", mark: "04", icon: "🖤", badge: "Живой пазл", desc: "3 фазы: воля, память, согласие", kicker: "ДЕНЬ 04 · ПРОТОКОЛ «МЫ»", title: "Протокол «МЫ»", instruction: "Освободи Грутика, восстанови фотографию и сплети рамку согласия.", photo: SITE_CONFIG.couplePhoto, rows: 4, cols: 3, relationship: SITE_CONFIG.relationship },
  ...
  var GREET = [
    ...
    [
      S(4),
      L("sonechka", "Грутик?.. Почему твой горшок смотрит на потолок?"),
      W(500),
      ...
      L("gru", "Я есть Грутик!", "Вообще-то дома сначала стучатся!"),
      L("sonechka", "Для начала поставь его на землю."),
      F("blobIn", 300),
      L("nar", "Щупальце разжимается. Грутик падает в горшок, но чёрная живая плёнка уже растекается по его коре."),
      FN(infect),
      F("blackout", 900),
      S(5),
      L("nar", "Белые глаза вспыхивают поверх знакомого лица. Корни и чёрные нити дёргаются в разные стороны."),
      L("venom", "Вы вчера сказали: «мы». Мы пришли."),
      L("gru", "Я есть Грутик.", "Но моя ветка всё ещё моя."),
      C("day4_venom", "Кому сейчас нужна твоя помощь?", [
        { text: "🌿 Грутик, смотри на меня. Ты здесь", steps: [
          L("nar", "Грутик цепляется взглядом за тебя. Дрожь в ветвях становится тише."),
          L("gru", "Я есть Грутик.", "Я здесь. Я — это я."),
          L("venom", "Мы слышим.")
        ]},
        { text: "🖤 Веном, разожми щупальце. Сейчас", steps: [
          L("nar", "Чёрная нить натягивается, будто собирается спорить, — и всё же медленно отпускает ветку."),
          L("venom", "Мы... разжали."),
          L("gru", "Я есть Грутик.", "Так намного лучше.")
        ]}
      ]),
      L("sonechka", "Быть «мы» — не значит забирать другого себе. Сначала научитесь быть рядом."),
      L("nar", "Грутик и Веном одновременно хватаются за световую проекцию памятной фотографии. Один тянет к себе. Второй не отпускает."),
      F("flash", 250),
      L("nar", "Проекция раскалывается на двенадцать фрагментов. Свет воспоминания гаснет под чёрными нитями."),
      L("gru", "Я есть Грутик...", "Это Вадим и Сонечка. Её нужно беречь."),
      L("venom", "Мы держали крепко."),
      L("gru", "Я есть Грутик!", "Слишком крепко!"),
      L("sonechka", "Сначала освободим ветви. Потом вернём воспоминание. И только после этого попробуем вместе."),
      B("Вернуть наше «мы»", markSeen(3))
    ],
    [
      S(7),
  ...
  var POSTWIN = [
    ...
    [
      S(5),
      L("nar", "Последний узел замыкается. Зелёные корни и чёрные нити удерживают одну рамку, не поглощая друг друга."),
      L("nar", "На нижнем краю мягко загорается дата: 24.07.24. Фотография снова цела."),
      L("venom", "Они разные. Но всё равно — «мы»."),
      L("gru", "Я есть Грутик.", "Рядом — не значит одинаковые."),
      L("venom", "Мы можем научиться."),
      L("gru", "Я есть Грутик.", "Оставайся пока. Только сначала спрашивай."),
      L("venom", "Можно помочь с колесом?"),
      L("gru", "Я есть Грутик!", "Теперь можно!"),
      FN(controlVenom),
      S(6),
      F("wheelIn"),
      L("nar", "Ветка толкает обод, а щупальце бережно поддерживает ось. Колесо начинает вращаться ровно и легко."),
      L("venom", "Вместе."),
      B("Крутить колесо")
    ],
    [
  ...
  var AFTERGIFT = [
    ...
    [
      S(6),
      L("nar", "Секретная коробка остаётся закрытой. Рядом в живой рамке светится восстановленная фотография."),
      L("venom", "Почему у Вадима и Сонечки на головах боевые конусы?"),
      L("gru", "Я есть Грутик.", "Это праздничные колпачки. Здесь так радуются вместе."),
      L("venom", "Нам нужен конус."),
      L("sonechka", "Какое слово мы сегодня учили?"),
      W(500),
      L("venom", "...Пожалуйста."),
      L("nar", "Тонкое щупальце складывается треугольником. Грутик примеряет импровизированный колпачок, а Веном осторожно придерживает рамку."),
      C("day4_after", "Что скажешь этой странной парочке?", [
        { text: "🎉 Вам идёт. Обоим", steps: [
          L("gru", "Я есть Грутик!", "Мы готовы к празднику!"),
          L("venom", "Боевой конус одобрен.")
        ]},
        { text: "🌙 Только спать будете не на потолке", steps: [
          L("venom", "Условие принято."),
          L("gru", "Я есть Грутик.", "Сегодня — точно в горшке.")
        ]}
      ]),
      L("nar", "У основания колеса на миг вспыхивает один тревожный огонёк. Веном замечает его первым."),
      L("venom", "Завтра посмотрим. Вместе."),
      B("До завтра!")
    ],
    [
      S(7),
